...
async function main() {
  console.log('\n🤖 GPT-4o Backtest with Excel Report\n');
  ...
  console.log(`Found ${validMatches.length} matches with odds data\n`);
  ...
  console.log('Processing matches with GPT-4o predictions...\n');
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    try {
      ...
      console.log(`   ⚠️ Prediction failed, skipping`);
      continue;
    }
    ...
    console.log(`   Result: ${homeScore}-${awayScore} | Predicted: ${prediction.matchOutcome.predicted} | ${outcomeCorrect ? '✅' : '❌'}`);

    // Rate limit for GPT API
  ...
  csvLines.push('GPT-4o BACKTEST REPORT');
  ...
  // Save CSV file
  const fileName = `GPT4o_Backtest_${new Date().toISOString().split('T')[0]}.csv`;
